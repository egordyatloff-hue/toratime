...
class ScheduleApp(App):
    ...
    def _confirm_alarms(self, times, msgs):
        ...
        self._rebuild_alarms_box()
        self._show_error("Установлено %d будильник(ов).\n\n"
    ...
    def reschedule_alarms(self):
        ...
        self._schedule_android_alarms(self._alarms)
        self._write_alarm_snapshot()
        self._show_error("Будильники обновлены.")
    ...
    def _grant_alarm_permissions(self):
        try:
            ...
            if Build.VERSION.SDK_INT >= 33:
                act.requestPermissions(["android.permission.POST_NOTIFICATIONS"], 1)
            if Build.VERSION.SDK_INT >= 31:
                ...
                if not am.canScheduleExactAlarms():
                    try:
                        ...
                        i.setData(autoclass("android.net.Uri").parse(
                            "package:%s" % act.getPackageName()))
        ...
        try:
            ...
            for a in alarms:
                if not a.get("on", True):
                    continue
                minutes = a["time"]
                action = "org.example.toratime.ALARM"
                intent = Intent(action)
                intent.setClassName("org.example.toratime", "org.example.toratime.AlarmReceiver")
                intent.putExtra("title", "Будильник")
                intent.putExtra("text", a.get("msg", "") or fmt_time(minutes))
                intent.putExtra("minutes", minutes)
                pi = PendingIntent.getBroadcast(act, minutes, intent,
                                                PendingIntent.FLAG_UPDATE_CURRENT
                                                | getattr(PendingIntent, "FLAG_IMMUTABLE",
                                                          PendingIntent.FLAG_UPDATE_CURRENT))
    ...
    def _cancel_android_alarm(self, a):
        try:
            ...
            act = PythonActivity.mActivity
            am = act.getSystemService(Context.ALARM_SERVICE)
            intent = Intent("org.example.toratime.ALARM")
            intent.setClassName("org.example.toratime", "org.example.toratime.AlarmReceiver")
            pi = PendingIntent.getBroadcast(act, a.get("time", 0), intent,
                                            PendingIntent.FLAG_UPDATE_CURRENT
                                            | getattr(PendingIntent, "FLAG_IMMUTABLE",
                                                      PendingIntent.FLAG_UPDATE_CURRENT))
            am.cancel(pi)
